...
df_details_finel = pd.DataFrame(columns=['Id', 'Latency','simulation_time', 'HRV_HF' ])
...
for i in range(len(dataECG)):
    ...
    df_details_finel = pd.concat([df_details_cut_ECG, df_details_finel], ignore_index=True)

    #Sorting the data df_details_finel on the arrangement of the simulation_time column according to size
    df_details_finel = df_details_finel.sort_values('simulation_time')
print(df_details_finel)
#df_details_finel.to_csv('df_details_finel_csv1.csv', index= False, header='true')


window = 200
df_details_finel['Moving_Avg_HRV_HF']=None
df_details_finel.loc[df_details_finel['Latency']=='Latency1','Moving_Avg_HRV_HF'] = df_details_finel[df_details_finel['Latency']=='Latency1'].HRV_HF.rolling(window).mean()
df_details_finel.loc[df_details_finel['Latency']=='Latency2','Moving_Avg_HRV_HF'] = df_details_finel[df_details_finel['Latency']=='Latency2'].HRV_HF.rolling(window).mean()
df_details_finel.loc[df_details_finel['Latency']=='Latency3','Moving_Avg_HRV_HF'] = df_details_finel[df_details_finel['Latency']=='Latency3'].HRV_HF.rolling(window).mean()


#pio.renderers.default = 'browser'
ECG_plot = px.line(df_details_finel, y='Moving_Avg_HRV_HF', x='simulation_time', color= df_details_finel.Latency)
ECG_plot.show()
