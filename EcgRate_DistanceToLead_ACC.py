...
old_path = 'G:\My Drive\Ariel Uni' #The path where the necessary information is stored in the Mobile-Lab
...
for i in range(len(ACC)):
    ...
    try:
        ...
        #Because the number of measurements in the data_preprocess data is greater
        #than the number of time points in the df_merged data. Basically, the number 
        #of "ECG_Rate" measurement points from data_preprocess needs to be adjusted
        #to the number of "SimulationTime" time points from df_merged, so we interpolate
        #between the 2 data:
...
#df_final.to_csv('df_finel_csv.csv', index= False, header='true')
...
#p1.to_csv('df_p1_csv.csv', index= False, header='true')
...
#p2.to_csv('df_p2_csv.csv', index= False, header='true')
...
#p3.to_csv('df_p3_csv.csv', index= False, header='true')
